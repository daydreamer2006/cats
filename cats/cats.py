...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    lists = [x for x in paragraphs if select(x)]
    if k > len(lists) - 1:
        return ''
    else:
        return lists[k]
...
def about(subject):
    ...
    def check(string):
        ...
        new_para = [lower(remove_punctuation(x)) for x in paragraph ]
        match = [x for x in new_para if x in subject]
        if len(match) == 0:
            return False
        else:
            return True
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    #find the smallest differenece
    #produce a new list of difference of the input and all the word in the wordlist

    diff_list = [diff_function(typed_word,word,limit) for word in word_list]
    result = min(diff_list, key = abs)
    if result > limit:
        return typed_word
    for i in range(len(word_list)):
        if result == diff_function(typed_word,word_list[i],limit):
            return word_list[i]
    #
    # END PROBLEM 5
...
print(furry_fixes("awful", "awesome", 2))
...
def time_per_word(words, timestamps_per_player):
    ...
    times =[]
    # BEGIN PROBLEM 9
    for n in range(len(tpp)):
        times_n = [tpp[n][i] - tpp[n][i - 1] for i in range(1, len(tpp[n]))]
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        for n in player_indices:
            time.append( get_time(times, n, i))
        mintime = min(time)
        for n in player_indices:
            if get_time(times, n, i) == mintime:
                player_word[n].append(words[i])
                break
        time = []
